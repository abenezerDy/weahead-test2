...
import { getStateTreeNode, isStateTreeNode, splitJsonPath, asArray, EMPTY_OBJECT, fail, isType, resolveNodeByPath, getRelativePathBetweenNodes, freeze, isModelType, InvalidReferenceError, normalizeIdentifier } from "../internal";
/**
...
export function onPatch(target, callback) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isStateTreeNode(target))
            throw fail("expected first argument to be a mobx-state-tree node, got " + target + " instead");
...
export function applyPatch(target, patch) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (typeof patch !== "object")
            throw fail("expected second argument to be an object or array, got " + patch + " instead");
    }
...
export function recordPatches(subject) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isStateTreeNode(subject))
            throw fail("expected first argument to be a mobx-state-tree node, got " + subject + " instead");
    }
    var disposer = null;
    function resume() {
        if (disposer)
...
export function hasParent(target, depth) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (depth < 0)
            throw fail("Invalid depth: " + depth + ", should be >= 1");
    }
...
export function hasParentOfType(target, type) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isStateTreeNode(target))
            throw fail("expected first argument to be a mobx-state-tree node, got " + target + " instead");
        if (!isType(type))
            throw fail("expected second argument to be a mobx-state-tree type, got " + type + " instead");
    }
    var parent = getStateTreeNode(target).parent;
    while (parent) {
...
export function resolveIdentifier(type, target, identifier) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isType(type))
            throw fail("expected first argument to be a mobx-state-tree type, got " + type + " instead");
        if (!isStateTreeNode(target))
            throw fail("expected second argument to be a mobx-state-tree node, got " + target + " instead");
        if (!(typeof identifier === "string" || typeof identifier === "number"))
            throw fail("expected third argument to be a string or number, got " + identifier + " instead");
    }
 ...
 * @returns
 */
export function tryReference(getter, checkIfAlive) {
 ...
 * @returns
 */
export function isValidReference(getter, checkIfAlive) {
    if (checkIfAlive === void 0) { checkIfAlive = true; }
    try {
        var node = getter();
        if (node === undefined || node === null) {
...
export function getRelativePath(base, target) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (!isStateTreeNode(base))
            throw fail("expected first argument to be a mobx-state-tree node, got " + base + " instead");
    }
...
export function clone(source, keepEnvironment) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isStateTreeNode(source))
            throw fail("expected first argument to be a mobx-state-tree node, got " + source + " instead");
    }
...
export function getPropertyMembers(typeOrNode) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isModelType(type))
            throw fail("expected a model type, but got " + type + " instead.");
    }
    return {
        name: type.name,
...
export function getMembers(target) {
    ...
    var reflected = tslib_1.__assign({}, getPropertyMembers(type), { actions: [], volatile: [], views: [] });
