...
function onPatch(target, callback) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isStateTreeNode(target))
            throw fail$1("expected first argument to be a mobx-state-tree node, got " + target + " instead");
        if (typeof callback !== "function")
            throw fail$1("expected second argument to be a function, got " + callback + " instead");
    }
...
function applyPatch(target, patch) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (typeof patch !== "object")
            throw fail$1("expected second argument to be an object or array, got " + patch + " instead");
    }
...
function recordPatches(subject) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isStateTreeNode(subject))
            throw fail$1("expected first argument to be a mobx-state-tree node, got " + subject + " instead");
    }
    var disposer = null;
    function resume() {
        if (disposer)
...
function hasParent(target, depth) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (typeof depth !== "number")
            throw fail$1("expected second argument to be a number, got " + depth + " instead");
        if (depth < 0)
            throw fail$1("Invalid depth: " + depth + ", should be >= 1");
    }
...
function hasParentOfType(target, type) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isStateTreeNode(target))
            throw fail$1("expected first argument to be a mobx-state-tree node, got " + target + " instead");
        if (!isType(type))
            throw fail$1("expected second argument to be a mobx-state-tree type, got " + type + " instead");
    }
    var parent = getStateTreeNode(target).parent;
    while (parent) {
...
function resolvePath(target, path) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (typeof path !== "string")
            throw fail$1("expected second argument to be a number, got " + path + " instead");
    }
...
function resolveIdentifier(type, target, identifier) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isType(type))
            throw fail$1("expected first argument to be a mobx-state-tree type, got " + type + " instead");
        if (!isStateTreeNode(target))
            throw fail$1("expected second argument to be a mobx-state-tree node, got " + target + " instead");
        if (!(typeof identifier === "string" || typeof identifier === "number"))
            throw fail$1("expected third argument to be a string or number, got " + identifier + " instead");
    }
...
function tryResolve(target, path) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isStateTreeNode(target))
            throw fail$1("expected first argument to be a mobx-state-tree node, got " + target + " instead");
        if (typeof path !== "string")
            throw fail$1("expected second argument to be a string, got " + path + " instead");
    }
...
function getRelativePath(base, target) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (!isStateTreeNode(base))
            throw fail$1("expected first argument to be a mobx-state-tree node, got " + base + " instead");
    }
...
function clone(source, keepEnvironment) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isStateTreeNode(source))
            throw fail$1("expected first argument to be a mobx-state-tree node, got " + source + " instead");
    }
...
function addDisposer(target, disposer) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (typeof disposer !== "function")
            throw fail$1("expected second argument to be a function, got " + disposer + " instead");
    }
    var node = getStateTreeNode(target);
...
function walk(target, processor) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (typeof processor !== "function")
            throw fail$1("expected second argument to be a function, got " + processor + " instead");
    }
    var node = getStateTreeNode(target);
...
function getPropertyMembers(typeOrNode) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isModelType(type))
            throw fail$1("expected a model type, but got " + type + " instead.");
    }
    return {
        name: type.name,
...
function applyAction(target, actions) {
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        if (typeof actions !== "object")
            throw fail$1("expected second argument to be an object or array, got " + actions + " instead");
    }
...
function getActionContext() {
    if (!currentActionContext)
        throw fail$1("Not running an action!");
    return currentActionContext;
}
/**
 * @internal
...
function tryCollectModelTypes(type, modelTypes) {
    ...
    if (subtypes) {
        ...
        for (var _i = 0, subtypesArray_1 = subtypesArray; _i < subtypesArray_1.length; _i++) {
            var subtype = subtypesArray_1[_i];
    ...
    MSTMap.prototype.put = function (value) {
        ...
        if (isStateTreeNode(value)) {
            ...
            if (mapType.identifierMode === MapIdentifierMode.YES) {
                var key = normalizeIdentifier(value[mapType.mapIdentifierAttribute]);
                this.set(key, value);
                return this.get(key);
            }
            throw fail$1(needsIdentifierError);
...
var MapType = /** @class */ (function (_super) {
    ...
    MapType.prototype.createNewInstance = function (node, childNodes) {
...
function array(subtype) {
    if (process.env.NODE_ENV !== "production") {
        if (!isType(subtype))
            throw fail$1("expected a mobx-state-tree type as first argument, got " + subtype + " instead");
    }
    ...
    ModelType.prototype.getChildType = function (propertyName) {
        if (process.env.NODE_ENV !== "production") {
            if (propertyName === undefined || propertyName === null) {
                throw fail$1("getChildType requires a property name when used over a model type");
            }
        }
...
function compose() {
    ...
    if (process.env.NODE_ENV !== "production") {
        args.forEach(function (type) {
            if (!isModelType(type))
                throw fail$1("expected a mobx-state-tree model type, got " + type + " instead");
        });
    }
    return args
...
function literal(value) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isPrimitive(value))
            throw fail$1("Literal types can be built only on top of primitives");
    }
...
var Refinement = /** @class */ (function (_super) {
    ...
    Refinement.prototype.reconcile = function (current, newValue) {
...
function refinement() {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (typeof name !== "string")
            throw fail$1("expected a string as first argument, got " + name + " instead");
        if (!isType(type))
            throw fail$1("expected a mobx-state-tree type as first or second argument, got " +
                type +
                " instead");
        if (typeof predicate !== "function")
            throw fail$1("expected a function as third argument, got " + predicate + " instead");
        if (typeof message !== "function")
            throw fail$1("expected a function as fourth argument, got " + message + " instead");
    }
    ...
    if (process.env.NODE_ENV !== "production") {
        realOptions.forEach(function (option) {
            if (typeof option !== "string")
                throw fail$1("expected all options to be string, got " + type + " instead");
        });
    }
...
function union(optionsOrType) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isType(optionsOrType) && !isPlainObject(optionsOrType))
            throw fail$1("First argument to types.union should either be a type, or an objects object of the form: { eager?: boolean, dispatcher?: Function }");
        types.forEach(function (type) {
            if (!isType(type))
                throw fail$1("expected all possible types to be a mobx-state-tree type, got " +
                    type +
                    " instead");
        });
    }
...
function checkOptionalPreconditions(type, defaultValueOrFunction) {
    ...
    if (process.env.NODE_ENV !== "production") {
        if (!isType(type))
            throw fail$1("expected a mobx-state-tree type as first argument, got " + type + " instead");
